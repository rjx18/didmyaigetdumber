...
const KPI_LIST = [
  ...
  { label: "Time to first token", pick: (s) => s.timing.ttft, roll: "avgTtftMs", fmt: "secs", goodDir: "down", section: "latencyTools" },
...
const SECTIONS = {
  ...
  tokensCache: {
    ...
    charts: [
      ...
      { kind: "bars", bars: "modelTokens", title: "Per-model token mix", sub: "total tokens per model · summed over the window" },
  ...
  latencyTools: {
    ...
    charts: [
      { kind: "line", title: "Time to first token", sub: "seconds", pick: (s) => s.timing.ttft, fmt: "secs", color: "accent", goodDir: "down" },
      { kind: "line", title: "Output throughput", sub: "tokens / sec", pick: (s) => s.timing.throughput, fmt: (v) => v.toFixed(0) + " tok/s", color: "ink", goodDir: "up" },
      { kind: "line", title: "Avg tool latency", sub: "ms · s when ≥ 1s", pick: (s) => s.timing.toolLatency, fmt: "msAdaptive", color: "ink", goodDir: "down" },
      { kind: "bars", bars: "toolMix", wide: true, title: "Tool mix — by invocations", sub: "share of all tool calls over the window" },
      { kind: "bars", bars: "toolErrors", title: "Error rate by tool", sub: "failures ÷ calls over the window" },
      { kind: "bars", bars: "toolOutput", title: "Tool mix — by output", sub: "output characters per tool over the window" },
